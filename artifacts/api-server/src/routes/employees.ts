...
router.get("/stats", async (_req, res) => {
  try {
    ...
    const totalSalary = all.reduce(
      (sum, e) => sum + parseFloat(e.baseSalary ?? "0"),
      0
    );
    const departments = [
      ...new Set(all.map((e) => e.department).filter(Boolean)),
    ];
...
router.get("/departments", async (_req, res) => {
  try {
    const rows = await db
      .selectDistinct({ department: employeesTable.department })
      .from(employeesTable)
      .where(sql`${employeesTable.department} is not null`);
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const salaries = await db
      ...
      .where(eq(salaryRecordsTable.employeeId, id))
  ...
  try {
    ...
    if (!employeeNumber) {
      ...
      while (!unique) {
        ...
        const existing = await db
          ...
          .from(employeesTable)
          .where(eq(employeesTable.employeeNumber, employeeNumber));
    ...
    const rows = await db
      ...
      .values({
        ...
        notes: body.notes,
        appointmentDocUrl: body.appointmentDocUrl,
        appointmentDocName: body.appointmentDocName,
    ...
    if (err?.message?.includes("unique")) {
      return res.status(409).json({ error: "رقم الموظف موجود مسبقاً" });
    }
...
router.post("/:id/documents", async (req, res) => {
  try {
    ...
    const rows = await db
      ...
      .values({ employeeId, docName, docType, docUrl })
      .returning();
    res.status(201).json(rows[0]);
  } catch {
