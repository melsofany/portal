import { Router } from "express";
      ...
      try {
        const { rows } = await pool.query(`
          SELECT
            ...
            cq.quotation_no,
            cq.request_date,
            cq.expiry_date   AS quotation_close_date,
    ...
    // GET /api/items/detail-by-code?code=...  — all records sharing the same internal code
    router.get("/detail-by-code", async (req, res) => {
      ...
      try {
        const { rows } = await pool.query(`
          SELECT
            ...
            cq.quotation_no,
            cq.request_date,
            cq.expiry_date                AS quotation_expiry_date,
            cq.status                     AS quotation_status,
            ...
            co.close_date                 AS order_close_date,
        ...
        // Supplier prices: join through canonical_items for this code
        const { rows: supplierRows } = await pool.query(`
          ...
          JOIN customer_quotation_items cqi2 ON (
            LOWER(TRIM(sqi.description)) = LOWER(TRIM(cqi2.description))
            OR (TRIM(sqi.part_no) <> '' AND LOWER(TRIM(sqi.part_no)) = LOWER(TRIM(cqi2.part_no)))
          )
          LEFT JOIN canonical_items ci_pno2
            ON TRIM(cqi2.part_no) <> ''
            AND LOWER(TRIM(ci_pno2.part_no)) = LOWER(TRIM(cqi2.part_no))
          LEFT JOIN canonical_items ci_desc2
            ON ci_pno2.id IS NULL
            AND (LOWER(TRIM(ci_desc2.description_en)) = LOWER(TRIM(cqi2.description))
              OR LOWER(TRIM(ci_desc2.description_ar)) = LOWER(TRIM(cqi2.description)))
          WHERE COALESCE(
            NULLIF(TRIM(cqi2.internal_code),''),
            ci_pno2.internal_code,
            ci_desc2.internal_code
          ) = $1
        ...
        const { rows: ciRows } = await pool.query(
          `SELECT description_en, description_ar, part_no, brand, category FROM canonical_items WHERE internal_code = $1 LIMIT 1`,
          [code]
        );
        const canonical = ciRows[0] ?? null;

        res.json({
          code,
          description: canonical?.description_en || rows[0]?.description || code,
          canonical,
          stats: {
